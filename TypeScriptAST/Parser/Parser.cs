...
using static Zu.TypeScript.TsParser.Utilities;
...
public class Parser
{
    ...
    public void InitializeState(
        ...
        /*IncrementalParser.SyntaxCursor*/object syntaxCursor,
    ...
    public bool IsIdentifier()
    {
        ...
        return Token() == SyntaxKind.AwaitKeyword && InAwaitContext() ? false : Token() > SyntaxKind.LastReservedWord;
    }
    ...
    {
        return Token() == t ? ParseTokenNode<T>(Token()) : (Node?)null;
    }
    ...
    {
        return ParseErrorBeforeNextFinishedNode ? null : null;
    ...
    public IEntityName ParseEntityName(bool allowReservedWords, DiagnosticMessage? diagnosticMessage = null)
    {
        ...
        {
            QualifiedName node = new() { Pos = entity.Pos };
    ...
    public NodeArray<TypeParameterDeclaration>? ParseTypeParameters()
    {
        return Token() == SyntaxKind.LessThanToken
            ? ParseBracketedList(TsTypes.ParsingContext.TypeParameters, ParseTypeParameter, SyntaxKind.LessThanToken, SyntaxKind.GreaterThanToken)
    ...
    {
        return Token() != SyntaxKind.OpenBracketToken ? false : LookAhead(IsUnambiguouslyIndexSignature);
    }
    ...
    {
        return InDisallowInContext() && Token() == SyntaxKind.InKeyword ? false : GetBinaryOperatorPrecedence() > 0;
    }
    ...
    public IJsxTagNameExpression ParseJsxElementName()
    {
        ...
        if (Token() == SyntaxKind.ThisKeyword)
        {
            ...
            while (ParseOptional(SyntaxKind.DotToken))
            {
                PropertyAccessExpression propertyAccess = new() { Pos = expression2.Pos }; //(PropertyAccessExpression)createNode(SyntaxKind.PropertyAccessExpression, expression.pos);
    ...
    {
        return IsHeritageClause() ? ParseList(TsTypes.ParsingContext.HeritageClauses, ParseHeritageClause) : (NodeArray<HeritageClause>?)null;
    }
